      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <section class="md:col-span-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="sp in paginatedProducts" :key="sp.id"
            ...
            <div>
              <img :src="sp.image" alt="" class="w-full h-40 object-contain mb-4 rounded-xl transition-transform hover:scale-105" />
  ...
  export default {
    ...
    data() {
      return {
        currentPage: 1,
        itemsPerPage: 8,
        filters: { priceMin: 0, priceMax: 60000000, category: null, brand: null },
        categoryOptions: [
    ...
    computed: {
      filteredProducts() {
        return this.products.filter(sp => {
          return (
            (!this.filters.category || sp.category === this.filters.category.value) &&
            (!this.filters.brand || sp.brand === this.filters.brand.value) &&
            sp.price >= this.filters.priceMin &&
            sp.price <= this.filters.priceMax
          );
        });
      },
      paginatedProducts() {
    ...
    methods: {
      ...
      resetFilters() {
        this.filters = { category: null, brand: null, priceMin: 0, priceMax: 60000000 };
        this.currentPage = 1;
      },
      muaNgay(sp) {
